...
//{28,1,5,24,16,2},{12,16,3,12,32,2},{6,32,2,5,32,1},{5,32,2,4,64,1},{4,64,2,3,64,1}
  {28,1,5,24,16,2},{12,16,3,12,16,2},{6,16,2,6,32,1},{6,32,2,5,32,1},{5,32,2,4,64,1}
},
{ /* CIFAR10(SLITx8 + BGRx3) */
//{32,3,5,28,11,2},{14,11,3,14,16,2},{7,16,2,7,32,1},{7,32,2,6,32,2},{3,32,2,2,64,1},{2,64,2,1,64,1}
// 3072             2156              784             1568            288             256
//{32,3,5,28,18,2},{14,18,3,14,16,2},{7,16,2,7,32,1},{7,32,2,6,32,2},{3,32,2,2,64,1},{2,64,2,1,64,1}
// 3072             3724              784             1568            288             256
  {32,3,5,28,18,2},{14,18,3,14,16,2},{7,16,2,7,32,1},{7,32,2,6,64,2},{3,64,2,2,64,1},{2,64,2,1,128,1}
// 3072             3724              784             1568            576             256
}
};
...
/*{200},{200},{200},{200},{200},{40},{10}  *//* FCeta=0.2f C4-F2                            | C5-F2 9000     V2-C5-F2 9000     | C6-F2 9000     V2-C6-F2 3783(48) */
  {200},{200},{200},{200},{200},{40},{10}    /* FCeta=0.4f C4-F2                            | C5-F2 9000     V2-C5-F2 9000     | C6-F2 3817(58) V2-C6-F2 3594(57) *//*★*/
...
void init_net(CNNet *net, int batch_size, struct c *c, struct f *f)
{
  ...
  /*       (conv)  multiply_float2D(tmp_dst:8x[100x24x24]   <-- Ki2h:8x[5x5], tmp_col[5x5]x[100x24x24])          ●Ki          *//* 1ch -> 8ch */
  /*                                                           (Ki2h=tmp_dst*tmp_col^Tと考える)                                */
  /*                                       V 出力時はch毎にkernが違うのでkernが外側                                            */
  /*       (conv)                     reshape 8x[100x24x24] --> nhidden:100x[8x24x24]                                          */
  /*                                                              V 入力時はch累算のためkernが内側                             */
  /*       (conv)  add_bias                                     nhidden:100x[8x24x24] hbias                      ●hbias       */
  /*       (conv)  relu                                         nhidden:100x[8x24x24]                            ◎            */
  /*       (conv)  maxpooling         npool:100x[8x12x12]   <-- nhidden:100x[8x24x24]                            ◎            */
  /*       (fc)    flat4Dto2D(nflat:100x[8x12x12]           <-- npool:100x[8x12x12])                                           */
  /*       (fc)    multiply_float2D(nout:100x10             <-- nflat:100x[8x12x12], Wh2o:[8x12x12]x[10])        ●Wh          *//* 8ch -> 10ch */
  /*       (fc)    repmat_add(nout:100x10,                  <-- obias:10をnoutに加算)               (Wh2o=nflat^T*noutと考える)*/
  /*       (fc)    softmax(nout:100x10)                                                                          ●obias       */
  /*===========================================================================================================================*/
  /* backward:                                                                                                                 */
  ...
  /*                                                    CNN-layer | FC-layer                                                   */
  /*                                                                                                                           */
  /*       (fc)    sum_rows(▲g_obias:10                    <-- nout:100x10のch毎の総和を戻す)                   ▲g_obias     */
  /*       (fc)    multiply_float2D(▲g_Wh2o:[8x12x12]x10   <-- nflat:100x[8x12x12]^T, nout:100x10)              ▲┌g_Wh      */
  /*       (fc)    multiply_float2D(nflat:100x[8x12x12]     <-- nout:100x10, Wh2o:[8x12x12]x10^T)                  └nflat⇔nout *//* 10ch -> 8ch */
  /*       (fc)    raise2Dto4D(npool:100x[8x12x12]          <-- nflat:100x[8x12x12])                                           */
  /*       (conv)  max_unpooling(nhiddenbak:100x[8x24x24]   <-- nhiddenbak, npoolbak:100x[8x12x12], npool:100x[8x12x12])◎     */
  /*       (conv)  relu_grad_merge(nhidden:100x[8x24x24]    <-- nhiddenbak:100x[8x24x24])                        ◎            */
  /*       (conv)  sum_rows4D(▲g_hbias:8                   <-- nhidden:100x[8x24x24]の100batch毎8x24x24の総和を戻す)▲g_hbias */
  ...
  /*       (conv)    multiply_float2D(tmp_col:25x[100x24x24] <-- Ki2h:8x25^T, tmp_dst:8x[100x24x24])               └tmpcol⇔dst *//* 8ch -> 1ch */
  /*       (conv)    pack_col2patch(ninput:■100x28x28       <-- tmp_col:25x[100x24x24], 5, 1)                                 */
  /*                                                      最外ループ:25                                                        */
  /*                ▲used for nn_update                    バッチループ:100                                                   */
  /*                                                          tmp_colの25x100の先頭毎(5x5の各img毎)に25x24x24のhidden値を加算  */
  ...
  /*    *(x1conv)  multiply_float2D(x1tmp_dst:64x[100x10x10]<-- x1Ki2h:64x[3x3x8], x1tmp_col:[3x3x8]x[100x10x10])●Ki          *//* 8ch -> 64ch */
  /*                                                            (x1Ki2h=tmp_dst*tmp_col^Tと考える)                             */
  /*                                       V 出力時はch毎にkernが違うのでkernが外側                                            */
  /*    *(x1conv)                     reshape 64x[100x10x10]--> x1hidden:100x[64x10x10]                                        */
  /*                                                              V 入力時はch累算のためkernが内側                             */
  /*    *(x1conv)  add_bias                                     x1hidden:100x[64x10x10] xibias                   ●            */                 /*簡単*/
  /*    *(x1conv)  relu                                         x1hidden:100x[64x10x10]                          ◎            */                 /*簡単*/
  /*    *(x1conv)  maxpooling        x1pool:100x[★64x5x5]  <-- x1hidden:100x[64x10x10]                          ◎            */                 /*簡単*/
  /*---------------------------------------------------------------------------------------------------------------------------*/
  /*       (fc)    flat4Dto2D(nflat:100x[★64x5x5]          <-- x1pool:100x[★64x5x5])                                         */
  /*       (fc)    multiply_float2D(nout:100x10             <-- nflat:100x[★64x5x5], Wh2o:[★64x5x5]x[10])      ●Wh          *//* 64ch -> 10ch */
  /*       (fc)    repmat_add(nout:100x10,                  <-- obias:10をnoutに加算)               (Wh2o=nflat^T*noutと考える)*/
  /*       (fc)    softmax(nout:100x10)                                                                          ●obias       */
  /*===========================================================================================================================*/
  /* backward:                                                                                                                 */
  /*       (fc)    sum_rows(▲g_obias:10                    <-- nout:100x10のch毎の総和を戻す)                   ▲g_obias     */
  /*       (fc)    multiply_float2D(▲g_Wh2o:[★64x5x5]x10  <-- nflat:100x[★64x5x5]^T, nout:100x10)             ▲┌g_Wh      */
  /*       (fc)    multiply_float2D(nflat:100x[★64x5x5]    <-- nout:100x10, Wh2o:[★64x5x5]x10^T)                 └nflat⇔nout *//* 10ch -> 64ch */
  /*       (fc)    raise2Dto4D(x1pool:100x[★64x5x5]        <-- nflat:100x[★64x5x5])                                          */
  /*---------------------------------------------------------------------------------------------------------------------------*/
  /*    *(x1conv)  max_unpooling(x1hiddenbak:100x[64x10x10] <-- x1hiddenbak, x1poolbak:100x[★64x5x5], x1pool:100x[★64x5x5])◎*/                 /*簡単*/
  /*    *(x1conv)  relu_grad_merge(x1hidden:100x[64x10x10]  <-- x1hiddenbak:100x[64x10x10])                      ◎            */                 /*簡単*/
  /*    *(x1conv)  sum_rows4D(▲g_x1bias:64                 <-- x1hidden:100x[64x10x10]の100batch毎64x10x10の総和を戻す)▲g_x1bias */             /*簡単*/
  /*    *(x1conv) (conv_backward(x1hidden, x1Ki2h, g_x1Ki2h, npool, 3, 1, x1tmp_col, x1tmp_dst))-------------------------------*/
  /*    *(x1conv)    unpack_patch2col(x1tmp_col:★3x3x8x[100x10x10]<-- npool:■100x[★8x12x12], 3, 1)                          *//* ■をchannel=1にする必要 */
  /*    *(x1conv)    reshape           x1hidden:100x64x10x10 --> x1tmp_dst:64x[100x10x10]                                      */
  /*    *(x1conv)    multiply_float2D(▲g_x1Ki2h:★64x[3x3x8]<-- x1tmp_dst:64x[100x10x10], x1tmp_col:★3x3x8x[100x10x10]^T])▲┌g_x1Ki*/
  /*    *(x1conv)    multiply_float2D(x1tmp_col:★3x3x8x[100x10x10]<-- x1Ki2h:64x[3x3x8]^T, x1tmp_dst:64x[100x10x10])         └tmpcol⇔dst *//* 64ch -> 8ch */
  /*    *(x1conv)    pack_col2patch(npool:■100x[★8x12x12] <-- x1tmp_col:★3x3x8x[100x10x10], 3, 1)                           *//* ■をchannel=1にする必要 */
  /*---------------------------------------------------------------------------------------------------------------------------*/
  /*       (conv)  max_unpooling(nhiddenbak:100x[8x24x24]   <-- nhiddenbak, npoolbak:100x[8x12x12], npool:100x[8x12x12])◎     */
  /*       (conv)  relu_grad_merge(nhidden:100x[8x24x24]    <-- nhiddenbak:100x[8x24x24])                        ◎            */
  /*       (conv)  sum_rows4D(▲g_hbias:8                   <-- nhidden:100x[8x24x24]の100batch毎8x24x24の総和を戻す)▲g_hbias */
  ...
  init_float4D(&(net->work),            batch_size,                       c[0].ichan,       c[0].isize,            c[0].isize);           /* batch=100, channel=1   28x28 */
  init_float4D(&(net->attention),       batch_size,                       c[0].ichan,       c[0].isize,            c[0].isize);           /* batch=100, channel=1   28x28 */
  for (l=0; l<CNN_DEPTH; l++) {
    init_float4D(&(net->ninput_dropoutmask[l]), 1,                        c[l].ichan,       c[l].isize,            c[l].isize);
  ...
  for (l=0; l<FC_DEPTH; l++) {
    init_float2D(&(net->nflat_dropoutmask[l]), 1,                  (l==0)?c[CNN_DEPTH-1].ochan*(c[CNN_DEPTH-1].osize/c[CNN_DEPTH-1].psize)*(c[CNN_DEPTH-1].osize/c[CNN_DEPTH-1].psize)
  ...
  init_random(0);
  for (l=0; l<CNN_DEPTH; l++)
    SampleGaussian(&(net->Ki2h[l]), 0, 0.01f); /* 最初は乱数で初期化 */
  for (l=0; l<FC_DEPTH; l++)
    SampleGaussian(&(net->Wh2o[l]), 0, 0.01f); /* 最初は乱数で初期化 */
}

void reset_dropoutmask(CNNet *net)
{
  int l;

  for (l=0; l<CNN_DEPTH; l++)
    reset_dropoutmask4D(&(net->ninput_dropoutmask[l]));
  for (l=0; l<FC_DEPTH; l++)
    reset_dropoutmask2D(&(net->nflat_dropoutmask[l]));
}

void reset_dropoutmask4D(float4D* n)
{
  int i;
  float *A = n->data;
  for (i=0;i<n->nstrides*n->nchannel*n->kstrides*n->stride_size;i++)
    *A++ = (rand() <= RAND_MAX*0.8) ? 1.0f : 0.0f;
}

void reset_dropoutmask2D(float2D* n)
{
  int i;
  float *A = n->data;
  for (i=0;i<n->nstrides*n->stride_size;i++)
    *A++ = (rand() <= RAND_MAX*0.8) ? 1.0f : 0.0f;
}

void dropout4D(float4D* n, float4D* mask)
{
#if 0
  return; /* do nothing */
#else
  int i, j;
  float *A = n->data;
  for (i=0;i<n->nstrides;i++) {
    float *B = mask->data;
    for (j=0;j<n->nchannel*n->kstrides*n->stride_size;j++)
      *A++ *= *B++;
  }
#endif
}

void dropout2D(float2D* n, float2D* mask)
{
#if 0
  return; /* do nothing */
#else
...
void conv_forward(int THREAD, int LANE, float4D *in, float2D *kernel, float4D *out, int ksize, float2D *tmp_col, float2D *tmp_dst)
{
  /* *unpack_patch2col                                      */
  /*   in:100x[8x12x12]                                     */
...
#endif
#if defined(EMAX7)
  monitor_time_start(THREAD, CONV_FORWARD_CNMUL); 
  xmax_conv_forward(THREAD, LANE, in, kernel, out, ksize);
  monitor_time_end(THREAD, CONV_FORWARD_CNMUL);
#else
  // unpack local patchs, stride=1
  monitor_time_start(THREAD, CONV_FORWARD_UNPACK);
  unpack_patch2col(tmp_col, in, ksize, kstride, oheight, owidth); // 5x5x(100x24x24)  <-100x28x28
  monitor_time_end(THREAD, CONV_FORWARD_UNPACK);                          // 3x3x8x[100x10x10]<-100x[8x12x12]
  monitor_time_start(THREAD, CONV_FORWARD_CNMUL); 
  multiply_float2D(THREAD, LANE, tmp_dst, kernel, 0, tmp_col, 0); // 8x[100x24x24]<-8x[5x5], [5x5]x[100x24x24]
  monitor_time_end(THREAD, CONV_FORWARD_CNMUL);             // 64x[100x10x10]<-64x[3x3x8], [3x3x8]x[100x10x10]

  // reshape 8x[100x24x24] --> 100x[8x24x24]
  // reshape 64x[100x10x10]--> 100x[64x10x10]
  monitor_time_start(THREAD, CONV_FORWARD_RESHAPE);
  ...
  monitor_time_end(THREAD, CONV_FORWARD_RESHAPE); //Nakashima
#endif
}
...
#define inference_sigwait \
            int signo;\
	    th_inference_args[THREAD].stat = 2; /* wait */\
	    sigwait(&th_inference_args[THREAD].sigset, &signo);\
...
{ /*--ここにhidden以降のみを使う細工が必要--*/
  ...
  monitor_time_start(THREAD, NN_FORWARD); /* Nakashima */
  ...
  if (spike_mode) {
    ...
    monitor_time_start(THREAD, NN_FORWARD_RELU);
    relu4(&(net->nhidden[0]));                                                 /* V1->c.nhidden[0].data */
    monitor_time_end(THREAD, NN_FORWARD_RELU);
    monitor_time_start(THREAD, NN_FORWARD_POOLING);
    max_pooling(&(net->npool[0]), &(net->nhidden[0]), c[0].psize, c[0].psize); /*   ->c.npool[0].data */
    monitor_time_end(THREAD, NN_FORWARD_POOLING);
    flat4Dto2D(&(net->nflat[0]), &(net->npool[0]));                            /*   ->f.nflat[0].data */

    monitor_time_start(THREAD, NN_FORWARD_FCMUL);
    smax_trial(0, net, c, f); /* IGNORE MACRO_PIPE */
    monitor_time_end(THREAD, NN_FORWARD_FCMUL);
  }
  else {
    int LANE = 0;
    for (l=0; l<CNN_DEPTH; l++) {
	...
	// first layer, conv, use stride=2
        /***********************ninput*** V CNN    */
        monitor_time_start(THREAD, CONV_FORWARD);
#ifndef CUDA
        //printf("th=%d l=%d LANE=%d conv_forward start\n", THREAD, l, LANE);
/*★IMAX3★*/conv_forward(THREAD, LANE, l==0?&(net->ninput):&(net->npool[l-1]), &(net->Ki2h[l]), &(net->nhidden[l]), c[l].ksize, &(net->tmp_col[l]), &(net->tmp_dst[l]));
        //printf("th=%d l=%d LANE=%d conv_forward end\n", THREAD, l, LANE);
#else
	conv_forward_cuda(l==0?&(net->ninput):&(net->npool[l-1]), &(net->Ki2h[l]), &(net->nhidden[l]), c[l].ksize, &(net->tmp_col[l]), &(net->tmp_dst[l]));
#endif
	monitor_time_end(THREAD, CONV_FORWARD);
      }
	...
	for (j=0;j<net->nhidden[l].nchannel;j++) {
      ...
      monitor_time_start(THREAD, NN_FORWARD_RELU);
      relu4(&(net->nhidden[l]));
      //printf("th=%d l=%d LANE=%d relu4 end\n", THREAD, l, LANE);
      monitor_time_end(THREAD, NN_FORWARD_RELU);
	...
	if (THREAD>0) th_inference_args[THREAD-1].deq = 1-th_inference_args[THREAD-1].deq; /* NHIDDEN/NPOOLを使ったDBUFの場合 */
...
/*★IMAX3★*/multiply_float2D(THREAD, LANE, &(net->nout[l]), &(net->nflat[l]), 0, &(net->Wh2o[l]), 0);
	...
	// softmax calculation
        monitor_time_start(THREAD, NN_FORWARD_SOFTMAX);
	softmax2D(&(net->nout[FC_DEPTH-1]), &(net->nout[FC_DEPTH-1]));
	monitor_time_end(THREAD, NN_FORWARD_SOFTMAX);
      }
    }
    /***************************************************************************/
    if (!MACROPIPE || NTHREAD==1 || eye_mode)	{
  ...
  if ((!MACROPIPE || NTHREAD==1 || eye_mode)
    ...
    //printf("th=%d copy result end\n", THREAD);
  }

  monitor_time_end(THREAD, NN_FORWARD); /* Nakashima */
}

void conv_backward(int THREAD, const float4D *out, const float2D *kernel,
                          ...
                          int ksize, float2D *tmp_col, float2D *tmp_dst)
{
  ...
  monitor_time_start(THREAD, CONV_BACKWARD_CNMUL1);
  xmax_conv_backward(THREAD, 0, out, kernel, g_kernel, in, ksize);
  monitor_time_end(THREAD, CONV_BACKWARD_CNMUL1);
#else
  // unpack local patchs, stride=1
  monitor_time_start(THREAD, CONV_BACKWARD_UNPACK);
  unpack_patch2col(tmp_col, in, ksize, kstride, oheight, owidth);
  monitor_time_end(THREAD, CONV_BACKWARD_UNPACK);

  // reshape 100x8x24x24 --> 8x100x24x24
  monitor_time_start(THREAD, CONV_BACKWARD_RESHAPE);
  for (i=0;i<nchannel;i++) {
    for (j=0;j<nbatch;j++) {
  ...
  monitor_time_end(THREAD, CONV_BACKWARD_RESHAPE);

  monitor_time_start(THREAD, CONV_BACKWARD_CNMUL1);
  multiply_float2D(THREAD, 0, g_kernel, tmp_dst, 0, tmp_col, 1);  // 8x25 dot 25x57600 --> 8x57600
  monitor_time_end(THREAD, CONV_BACKWARD_CNMUL1);
  monitor_time_start(THREAD, CONV_BACKWARD_CNMUL2); // Nakashima
  multiply_float2D(THREAD, 0, tmp_col, kernel, 1, tmp_dst, 0);
  monitor_time_end(THREAD, CONV_BACKWARD_CNMUL2);

  monitor_time_start(THREAD, CONV_BACKWARD_PACK);
  pack_col2patch(in, tmp_col, ksize, kstride, oheight, owidth);
  monitor_time_end(THREAD, CONV_BACKWARD_PACK);
#endif
}

void nn_backprop(int THREAD, CNNet *net, struct c *c, struct f *f, float2D *gradout, int spike_mode)
{ /*--ここにhidden以降のみを使う細工が必要--*/
  float *A, *B;
  int i, l;

  monitor_time_start(THREAD, NN_BACKWARD); /* Nakashima */

  // copy gradient to output player
  copy2D(&(net->nout[FC_DEPTH-1]), gradout);

  if (spike_mode) {
    monitor_time_start(0, NN_BACKWARD_FCMUL1);
    smax_train(net, c, f);
    monitor_time_end(0, NN_BACKWARD_FCMUL1);
  }
  else {
    for (l=FC_DEPTH-1; l>=0; l--) {
      ...
      monitor_time_start(THREAD, NN_BACKWARD_FCMUL1);
      multiply_float2D(THREAD, 0, &(net->g_Wh2o[l]), &(net->nflat[l]), 1, &(net->nout[l]), 0);
      monitor_time_end(THREAD, NN_BACKWARD_FCMUL1);

      // backprop to previous layer
      monitor_time_start(THREAD, NN_BACKWARD_FCMUL2);
      multiply_float2D(THREAD, 0, &(net->nflat[l]), &(net->nout[l]), 0, &(net->Wh2o[l]), 1);
      dropout2D(&(net->nflat[l]), &(net->nflat_dropoutmask[l])); // nflat:★DROPOUT対象
      monitor_time_end(THREAD, NN_BACKWARD_FCMUL2);
	...
	// backprop to pooling layer
        // unpool
        monitor_time_start(THREAD, NN_BACKWARD_UNPOOLING);
	max_unpooling(&(net->nhiddenbak[l]), &(net->nhiddenbak[l]), &(net->npoolbak[l]), &(net->npool[l]), c[l].psize, c[l].psize);
	monitor_time_end(THREAD, NN_BACKWARD_UNPOOLING);

	// calculate gradient of relu layer
        // nhidden = F<relu_grad>(nhidden) * nhiddenbak;
	monitor_time_start(THREAD, NN_BACKWARD_RELU);
	relu_grad_merge4(&(net->nhidden[l]), &(net->nhiddenbak[l]));
	monitor_time_end(THREAD, NN_BACKWARD_RELU);

	// call grad of layer1
        // g_hbias = sumall_except_dim<2> (nhidden);
	sum_rows4D(&(net->g_hbias[l]), &(net->nhidden[l]));
	monitor_time_start(THREAD, CONV_BACKWARD);
	conv_backward(/*THREAD*/0, &(net->nhidden[l]), &(net->Ki2h[l]), &(net->g_Ki2h[l]), l==0?&(net->ninput):&(net->npool[l-1]), c[l].ksize, &(net->tmp_col[l]), &(net->tmp_dst[l]));
	dropout4D(l==0?&(net->ninput):&(net->npool[l-1]), &(net->ninput_dropoutmask[l])); // ninput/npool:★DROPOUT対象
	monitor_time_end(THREAD, CONV_BACKWARD);
                                                           /***********************ninput*** A CNN    */
      }
    }
  }

  monitor_time_end(THREAD, NN_BACKWARD); /* Nakashima */
  return;
}

void nn_update(int THREAD, CNNet *net, float cnn_eta, float fc_eta, float wd, int spike_mode)
{ /*--ここにhidden以降のみを使う細工が必要--*/
  int l, i, j;
  float *A, *B;

  monitor_time_start(THREAD, NN_UPDATE); /* Nakashima */

  if (spike_mode)
    smax_update(net, fc_eta, wd);
  else {
#define WEIGHTLIMIT 1.0
    for (l=0; l<CNN_DEPTH; l++) {
      if (l>0 || cnn_mode) {
	// update weight
        A = net->Ki2h[l].data;
	...
	for (i=0;i<net->Ki2h[l].nstrides*net->Ki2h[l].stride_size;i++, A++, B++) {
	  *A -= cnn_eta * ( wd * *A + *B);
	  if      (*A < -WEIGHTLIMIT) *A = -WEIGHTLIMIT;
	  else if (*A >  WEIGHTLIMIT) *A =  WEIGHTLIMIT;
	}
      }
  ...
  monitor_time_end(THREAD, NN_UPDATE); /* Nakashima */
}
