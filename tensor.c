...
void multiply_float2D(int THREAD, int LANE, float2D *C, const float2D *A, int transA, const float2D *B, int transB) /* C=AxB, transA,or B */ {
  ...
  if (!transA && !transB) /* A: m ¹Ô k Îó  B: k ¹Ô n Îó  C: m ¹Ô n Îó */
    xmax_sgemm00(THREAD, LANE, m, n, ka, A->data, B->data, C->data);
  else if (transA && !transB) /* A: k ¹Ô m Îó  B: k ¹Ô n Îó  C: m ¹Ô n Îó */
    xmax_sgemm10(THREAD, LANE, m, n, ka, A->data, B->data, C->data);
  else if (!transA && transB) /* A: m ¹Ô k Îó  B: n ¹Ô k Îó  C: m ¹Ô n Îó */
    xmax_sgemm01(THREAD, LANE, m, n, ka, A->data, B->data, C->data);
