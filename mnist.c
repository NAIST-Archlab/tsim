...
void Ipl2F4h(int nx, int w, int h, unsigned int *slit, unsigned int *corn, unsigned int *img, float4D *xdata)
{
//  . . . . . . . . . .
//  . . . . . . . . . . ⇒WDxHT -> 28x28 x 10x10
//  . . . . . . . . . .
...
//  . . . . . . . . . .   boxsize = 28
  ...
  for(b=0;b<nBatch;b++){
	...
	unsigned int  cpix   = corn[(b/nx)*w*nY4 + (h-nY4*nBatch/nx)/2*w + (b%nx)*nX4 + (w-nX4*nx)/2 + (y+2)*w+(x+2)]; /* 100batch x 24x24 x 24bit (B:8bit|G:8bit|R:8bit) */
	unsigned char cbyte  = cpix>>24|cpix>>16|cpix>>8;                                                              /* 100batch x 24x24 x 8bit */
	...
	case 8:
	  ...
	  break;
        case 16: /* for MNIST */
          /* slit angles(#0-#7) + corners(#8-15) */
          for(c=0;c<8;c++){
            xdata->data[b*nChannels*nY*nX+ c   *nY*nX+y*nX+x] = ((sbyte>>c)&1) ? 1.0f : 0.0f;
          }
          for(c=0;c<7;c++){ /* #15 is ignored */
            xdata->data[b*nChannels*nY*nX+(c+8)*nY*nX+y*nX+x] = ((cbyte>>c)&1) ? 1.0f : 0.0f;
          }
          /* brightness is in channel (#15) */
