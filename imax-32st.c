...
#if defined(EMAX7)
#if defined(ARMZYNQ)
  if ((NLANE = emax7_open(4)) == NULL) /* EMAX7 MACRO_PIPELIING (tsim-zynq,tsim-zynq.emax7*) */
    exit(1);
#else
  NLANE = 4; /* EMAX7 MACRO_PIPELIING (tsim-bsd.emax7nc,tsim-cent.emax7nc) */
#endif
#endif
...
//#define XMAX_VALIDATE
    ...
    xmax_cpyin(0, i_ker[LANE], &K,  ker, IC,    OC,  K, K, 1); /* 出力のみ回収すればよい */
...
#ifdef ARMZYNQ
//pthread_mutex_lock(&axi_dma_mutex);/*****AXIの干渉と思われる問題の回避*****/
#endif
      ...
      /*3*/ for (CHIP=0; CHIP<NCHIP; CHIP++) { /* output channels are parallelized by multi-chip (OC4/#chip) */
        /*2*/ for (INIT1=1,LOOP1=RMGRP,rofs=(0-IM4)<<32|((0-M4)&0xffffffff); LOOP1--; INIT1=0) {                      /* mapped to FOR() on BR[63][1][0] */ /* stage#0 */
          /*1*/ for (INIT0=1,LOOP0=M,cofs=(0-4LL)<<32|((0-4LL)&0xffffffff); LOOP0--; INIT0=0) {                       /* mapped to FOR() on BR[63][0][0] */ /* stage#0 */
                  ...
                  /****in0*****/
                  mop(OP_LDWR,   1, &BR[2][0][1],  (Ull)kp00[CHIP], 0LL, MSK_D0, (Ull)i_ker[LANE], Klen, 0, Force, (Ull)NULL, Klen); /* stage#2 */
                  mop(OP_LDWR,   1, &BR[2][0][0],  (Ull)kp01[CHIP], 0LL, MSK_D0, (Ull)i_ker[LANE], Klen, 0, Force, (Ull)NULL, Klen); /* stage#2 */
                  mop(OP_LDWR,   1, &BR[2][1][1],  (Ull)kp02[CHIP], 0LL, MSK_D0, (Ull)i_ker[LANE], Klen, 0, Force, (Ull)NULL, Klen); /* stage#2 */
                  mop(OP_LDWR,   1, &BR[2][1][0],  (Ull)kp03[CHIP], 0LL, MSK_D0, (Ull)i_ker[LANE], Klen, 0, Force, (Ull)NULL, Klen); /* stage#2 10KB */
...
//EMAX5A end
#ifdef ARMZYNQ
//pthread_mutex_unlock(&axi_dma_mutex);/*****AXIの干渉と思われる問題の回避*****/
#endif
printf("5");
//fflush(stdout);
            if (Force) Force = 0;
...
//EMAX5A drain_dirty_lmm
	...
	for (rofs=0; rofs<M; rofs++) {
	  for (cofs=0; cofs<M; cofs++) {
	    ...
	    float xmax = *(float*)&i_out[LANE][((img*OC4+oc)*M+rofs)*M+cofs];
...
/**/    /*2*/ for (img=0; img<BATCH; img++) {
    ...
    monitor_time_start(THREAD, IMAX_CPYIN); 
    xmax_cpyin(1, i_inp[LANE], &IM, in0, BATCH, IC, IM, M, K); /* この時点で0.0のPADを追加できる */
    xmax_cpyin(0, i_ker[LANE], &K,  ker, IC,    OC,  K, K, 1); /* 出力のみ回収すればよい */
    xmax_bzero(i_out[LANE], BATCH*OC4*M*M);
    monitor_time_end(THREAD, IMAX_CPYIN); 
    IM4   = IM*4;
...
//EMAX5A drain_dirty_lmm
	...
	for (rofs=0; rofs<M; rofs++) {
	  for (cofs=0; cofs<M; cofs++) {
	    float host = *(float*)&out0[((img*OC+oc)*M+rofs)*M+cofs];
	    float xmax = *(float*)&i_out[LANE][((oc*M+rofs)*BATCH+img)*M+cofs];
	    if (udiff(host,xmax)>ERRTH) {
	      count++;
...
//#define XMAX_VALIDATE
  ...
  N = (n+3)&~3;
  monitor_time_start(THREAD, IMAX_CPYIN); 
  xmax_cpyin(2, i_m0A[LANE], &m, A, 1, 1, m, ka, 1);
  xmax_cpyin(2, i_m0B[LANE], &n, B, 1, 1, n, ka, 1);
...
//EMAX5A drain_dirty_lmm
  ...
  for (row=0; row<m; row++) {
    for (col=0; col<n; col++) {
      if (udiff(C[row*n+col],*(float*)&i_m0C[LANE][row*n+col])>ERRTH) {
	count++;
	printf("[%d][%d]: C=%7.5e i_m0C=%7.5e\n", row, col, C[row*n+col], *(float*)&i_m0C[LANE][row*n+col]);
      }
    }
  }
  if (count)
...
#define PBL1_4_VERSION1
  ...
  monitor_time_start(THREAD, IMAX_CPYIN);
  xmax_cpyin(0, i_out[LANE], &M,  out0, BATCH, OC,  M, M, 1); //imemcpy(i_out[LANE], out0, BATCH*OC*M*M);   M=M;
  xmax_cpyin(0, i_inp[LANE], &IMX, in0, BATCH, IC, IM, M, K); //imemcpy(i_inp[LANE], in0,  BATCH*IC*IM*IM); IMX=IM;
  ...
  for (img=0; img<BATCH; img++) {
    for (ic=0; ic<IC; ic++) { /* set offset of input channel */
      for (oc=0; oc<OC; oc++) { /* set output channel */
	op0 = &i_out[LANE][(img*OC+oc)*M*M]; /* top of output image */
	kp = &i_ker[LANE][(oc*IC+ic)*K*K];
	kidx = 0;
	for (y=-(K/2); y<K-(K/2); y++) { /* kernel loop */
	  for (x=-(K/2); x<K-(K/2); x++) {
	    ip0 = &i_inp[LANE][(img*IC+ic)*IMX*IMX+(y+K/2)*IMX+(x+K/2)]; /* top of input */
	    kp0 = kp+kidx;
	    for (rofs=0; rofs<M; rofs++) {
	      for (cofs=0; cofs<M; cofs++) { /* image loop (cofs) */
...
//EMAX5A drain_dirty_lmm
    ...
    for (oc=0; oc<OC; oc++) {
      for (y=0; y<K; y++) {
        for (x=0; x<K; x++) {
          ...
          float xmax = *(float*)&i_ker[LANE][(oc*IC+ic)*K*K+y*K+x];
          if (udiff(host,xmax)>ERRTH) {
            count++;
  ...
  /***********************************/
  monitor_time_start(THREAD, IMAX_CPYIN);
  xmax_cpyin(1, i_out[LANE], &M,  out0, BATCH, OC,  M, M, 1); //dst[OC][M][BATCH][M]     <- src[BATCH][OC][M][M]
  xmax_cpyin(1, i_inp[LANE], &IMX, in0, BATCH, IC, IM, M, K); //dst[IC][IMX][BATCH][IMX] <- src[BATCH][IC][IM][IM]
  xmax_bzero(i_ker[LANE], OC*IC*K*K); /* g_kernel */
  monitor_time_end(THREAD, IMAX_CPYIN);
#if 0
  for (oset=0; oset<((OC+OMAP-1)&~(OMAP-1)); oset+=OMAP) { /* set output channel */
	...
	for (y=-(K/2); y<K-(K/2); y++) { /* kernel loop */
	  for (x=-(K/2); x<K-(K/2); x++) {
	    for (ic=0; ic<IMAP; ic++)
	      ip0[ic] = (iset+ic)<IC ? &i_inp[LANE][(iset+ic)*IMX*BATCH*IMX+(rofs+y+K/2)*BATCH*IMX+(x+K/2)] : 0; /* input */
	    for (oc=0; oc<OMAP; oc++)
	      op0[oc] = (oset+oc)<OC ? &i_out[LANE][(oset+oc)*M*BATCH*M+rofs*BATCH*M] : 0; /* output */
	    for (ic=0; ic<IMAP; ic++) {
	      for (oc=0; oc<OMAP; oc++)
		kp0[oc][ic] = ((iset+ic)<IC && (oset+oc)<OC) ? &i_ker[LANE][((oset+oc)*IC+iset+ic)*K*K+kidx] : 0; /* NULL skip DMA */
	    }
...
#define PBL1_5_VERSION1
  ...
  monitor_time_start(THREAD, IMAX_CPYIN);
//xmax_cpyin(0, i_out[LANE], &M, out0, BATCH, OC, M, M, 1); /* out *//*★★★削除可能★★★*/
  xmax_cpyin(0, i_ker[LANE], &K, ker,  IC,    OC, K, K, 1); //imemcpy(i_ker[LANE], ker,  OC*IC*K*K); K=K;
  xmax_bzero(i_inp[LANE], BATCH*IC*IM*IM); /* in */
  monitor_time_end(THREAD, IMAX_CPYIN);
#if 0
  if (K == 1 || IM-K+1 == M) { y0 = 0;    x0 = 0;    }
  else if (IM == M)          { y0 = -K/2; x0 = -K/2; }
  for (img=0;img<BATCH;img++) { /*100, 100*/
    for (ch=0;ch<IC*K*K;ch++) { /*5x5, 8x3x3*/
      ...
      x  = ch%(K*K)%K + x0;
      for (oc=0; oc<OC; oc++) {
	op0 = &i_out[LANE][img*M*M*OC+oc*M*M];
	ip0 = &i_inp[LANE][(img*IC+ic)*IM*IM+y*IM+x];
	float cker = *(float*)&i_ker[LANE][oc*IC*K*K+ch];
	for (rofs=0;rofs<M;rofs++) { /*24, 10*/
	  for (cofs=0;cofs<M;cofs++) { /*24, 10*/
	    if (0<=rofs+y && rofs+y<IM && 0<=cofs+x && cofs+x<IM)
...
printf("   BACK10  IMlen=%dB Mlen=%dB\n", (Uint)IMlen*4, (Uint)Mlen*4);
  ...
  for (img=0;img<BATCH;img++) { /*100, 100*/
	...
	op0 = &i_out[LANE][img*M*M*OC+oc*M*M];
	ip0 = &i_inp[LANE][(img*IC+ic)*IM*IM+y*IM+x];
	c00 = (Ull)i_ker[LANE][oc*IC*K*K+ch];
	Uint *ot00 = op0;                      
	Uint *it00 = &i_inp[LANE][(img*IC+ic)*IM*IM];
    ...
    for (ic=0; ic<IC; ic++) {
      for (y=0; y<IM; y++) {
        for (x=0; x<IM; x++) {
          ...
          float xmax = *(float*)&i_inp[LANE][(img*IC+ic)*IM*IM+y*IM+x];
          if (udiff(host,xmax)>ERRTH) {
            count++;
...
#endif
  monitor_time_start(THREAD, IMAX_CPYOUT);
  xmax_cpyout(2, in0, 1, 1, i_inp[LANE], BATCH*IM*IM, IC, IC); /* in */
  monitor_time_end(THREAD, IMAX_CPYOUT);
#endif
...
//xmax_cpyin(1, i_out[LANE], &M, out0, BATCH, OC, M, M, 1); //dst[OC][M][BATCH][M] <- src[BATCH][OC][M][M]
  ...
  for (oset=0; oset<((OC+OMAP-1)&~(OMAP-1)); oset+=OMAP) { /* set output channel */
	...
	if (0<=rofs+y && rofs+y<IM) {
	  ip0 = &i_inp[LANE][ic*IM*BATCH*IM+(rofs+y)*BATCH*IM+x];
	  for (oc=0; oc<OMAP&&(oset+oc)<OC; oc++) { /* set output channel */
	    op0[oc] = &i_out[LANE][(oset+oc)*M*BATCH*M+rofs*BATCH*M];
	    kp0[oc] = *(float*)&i_ker[LANE][(oset+oc)*IC*K*K+ch];
	    for (img=0;img<BATCH;img++) { /*100, 100*/
	      for (cofs=0;cofs<M;cofs++) { /*24, 10*/
...
//EMAX5A drain_dirty_lmm
  ...
  for (img=0; img<BATCH; img++) {
	...
	for (x=0; x<IM; x++) {
	  float host = *(float*)&in0[(img*IC+ic)*IM*IM+y*IM+x];
	  float xmax = *(float*)&i_inp[LANE][ic*IM*BATCH*IM+y*BATCH*IM+img*IM+x];
	  if (udiff(host,xmax)>ERRTH) {
	    count++;
